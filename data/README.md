...
full_join(spots$AAR, spots$coords, by = "spot") %>%
  mutate(AARs = factor(AARs, levels = c("Red pulp", "B follicle", "Marginal zone", "PALS"))) %>%
  ggplot() + 
  geom_point(aes(x = imagerow, y = imagecol, color = AARs), size = 0.5) +
  labs(color = "") +
  theme_bw() +
...
```
![github-small](https://github.com/nygctech/INLAomics/blob/main/data/spleen.png)